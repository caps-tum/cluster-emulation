...
if [ $# -ne 2 ]
  then
    ...
    echo "first place a *.swf file under ./build/slurm-plugins"
    echo "then drop into the container, and:"
...
echo "copying dummy scheduler plugin to disable thread creation..."
cp -a ./build/bin/simulator-input/sched_dummy.so ./install/slurm/lib/slurm/
cp -a ./build/bin/simulator-input/sched_dummy.a  ./install/slurm/lib/slurm/
cp -a ./build/bin/simulator-input/sched_dummy.la ./install/slurm/lib/slurm/
